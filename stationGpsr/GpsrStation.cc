...
void GpsrStation::finish(){
    if(!printed){
        ...
        if (file.is_open()) {
            file << data.dump(4); // Indentazione di 4 spazi
            file.close();
...
/*
    Coord destinationPosition = globalPositionTable.getPosition(destinationAddress);
    Coord myPosition = mobility->getCurrentPosition();

    double bestDistance = (destinationPosition - myPosition).length();
    Coord bestPosition = myPosition;

    double worstDistance = bestDistance;
    Coord worstPosition = bestPosition;
    ...
    for (auto& neighborAddress : neighborAddresses) {
        if (neighborAddress != destinationAddress){
            ...
            if (neighborDistance > worstDistance){
                worstDistance = neighborDistance;
                worstPosition = neighborPosition;
            }
        }
    }
    if ((destinationPosition - myPosition).length() < distance){
        Coord direction = (destinationPosition - worstPosition).normalize();
        double epsilon = 10.0;
        Coord f = Coord();
        f.setX(worstPosition.getX() - (direction.getX()*epsilon));
        f.setY(worstPosition.getY() - (direction.getY()*epsilon));
        f.setZ(worstPosition.getZ() - (direction.getZ()*epsilon));
        return f;
    }
    else{
        Coord direction = (destinationPosition - bestPosition).normalize();
        double epsilon = 10.0;
        Coord f = Coord();
        f.setX(bestPosition.getX() + (direction.getX()*epsilon));
        f.setY(bestPosition.getY() + (direction.getY()*epsilon));
        f.setZ(bestPosition.getZ() + (direction.getZ()*epsilon));
        return f;
    }
    */
    double prob = uniform(0.0, 1.0);
           if (prob <= 0.33){
               ...
               // std::cout << "Opposite map: real = " << pos << ", false = " << f << endl;
               ...
               // std::cout << "Random: real = " << mobility->getCurrentPosition() << ", false = " << f << endl;
...
const Ptr<GpsrBeacon> GpsrStation::createBeacon()
{
    const auto& beacon = makeShared<GpsrBeacon>();
        // MODIFICA
    if (isMalicious){ //&destinationSetted
        beacon->setAddress(getSelfAddress());
        beacon->setPosition(generateFalsePosition());
...
INetfilter::IHook::Result GpsrStation::datagramPreRoutingHook(Packet *datagram)
{
    ...
    if (destination.isMulticast() || destination.isBroadcast() || routingTable->isLocalAddress(destination))
        ...
        //std::cout << datagram->getFullName() << " TTL: " << ipv4Header->getTimeToLive() << endl;
...
{
    //std::cout << datagram->getInfoStringRepresentation() << endl;
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
    const L3Address& destination = networkHeader->getDestinationAddress();
...
void GpsrStation::handleStartOperation(LifecycleOperation *operation)
{
    ...
    storeSelfPositionInGlobalRegistry();
    if(isStation){
        stationMap[getStationIndex(mobility->getCurrentPosition())] = getSelfAddress();
        schedulePurgeStationTimer();
    }
    else {
