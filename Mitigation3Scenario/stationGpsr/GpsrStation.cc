...
std::map<int, std::vector<unsigned char>> GpsrStation::publicKeyStationMapEDDSA;
std::map<L3Address, std::vector<unsigned char>> GpsrStation::publicKeyDroneMapEDDSA;

std::map<int, EVP_PKEY*> GpsrStation::keyPairStationMapECDSA;
std::map<L3Address, EVP_PKEY*> GpsrStation::keyPairDroneMapECDSA;

std::vector<std::string> GpsrStation::maliciousStations;
...
GpsrStation::~GpsrStation()
{
    ...
    cancelAndDelete(sendPositionRequestMessage);
...
void GpsrStation::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        publicKeyDroneMapEDDSA.clear();
        publicKeyStationMapEDDSA.clear();

        keyPairStationMapECDSA.clear();
        keyPairDroneMapECDSA.clear();
        ...
        sendPositionRequestMessage = new cMessage("SendPositionRequestMessage");
...
void GpsrStation::generateKeysEDDSA() {
    ...
    if (crypto_sign_keypair(publicKeyEDDSA.data(), privateKeyEDDSA.data()) != 0) {
        throw std::runtime_error("Errore nella generazione delle chiavi EDDSA");
    }
}
...
std::string GpsrStation::generateFakeSignatureEDDSA(std::string messageString, uint64_t nonce) {
    std::vector<unsigned char> message(messageString.begin(), messageString.end());
    std::vector<unsigned char> messageWithNonce(sizeof(nonce) + message.size());

    std::vector<unsigned char> fakeSignature(crypto_sign_BYTES + messageWithNonce.size());

    randombytes_buf(fakeSignature.data(), fakeSignature.size());

    return base64_encode(fakeSignature.data(), fakeSignature.size());
}
...
const Ptr<StationNotice> GpsrStation::createStationNotice(bool deregisterFlag)
{
    ...
    std::string clear = std::to_string(pos.getX()) + "-" + std::to_string(pos.getY()) + "-" + std::to_string(pos.getZ());
    uint64_t nonce = nonceGen.generateNonce();
    if (signAlgorithm == ECDSA){
        stationNotice->setDroneSignature(signMessageECDSA(keyPairECDSA, clear, nonce).c_str());
    }
    else{
        stationNotice->setDroneSignature(signMessageEDDSA(clear, privateKeyEDDSA, nonce).c_str());
    }
    stationNotice->setDroneNonce(nonce);

    stationNotice->setChunkLength(B(getSelfAddress().getAddressType()->getAddressByteLength()
            + positionByteLength
            + 1
            + sizeof(stationNotice->getDroneSignature())
            + sizeof(stationNotice->getDroneNonce())
            ));
...
void GpsrStation::processStationNotice(Packet *packet){
    ...
    std::string droneSignature = req->getDroneSignature();
    uint64_t droneNonce = req->getDroneNonce();
    if(req->getDeregister()){
        if(quadNodesPositionTable.hasPosition(address)){
            quadNodesPositionTable.removePosition(address);
            if(quadNodesSignatureMap.find(address) != quadNodesSignatureMap.end()){
                quadNodesSignatureMap.erase(address);
            }
        }
    }
    else{
        ...
        bool verified = false;
        if (signAlgorithm == ECDSA){
            verified = verifySignatureECDSA(keyPairDroneMapECDSA[address], clear, droneNonce, droneSignature);
        }
        else{
            verified = verifySignatureEDDSA(clear, droneSignature, publicKeyDroneMapEDDSA[address], droneNonce);
        }
        if(verified) {
            quadNodesPositionTable.setPosition(address, position);
            std::tuple<Coord, std::string, uint64_t, simtime_t> tupla(position, droneSignature, droneNonce, simTime());
            quadNodesSignatureMap[address] = tupla;
        }
        else{
            if(quadNodesSignatureMap.find(address) != quadNodesSignatureMap.end()){
                quadNodesSignatureMap.erase(address);
...
Coord GpsrStation::generateFalsePositionStation(){
    ...
    if (prob <= 0.33){
        ...
        std::cout << "Opposite quad: real = " << pos << ", false = " << falsePosition << endl;
...
const Ptr<PositionResponse> GpsrStation::createPositionResponse(L3Address address)
{
    ...
    if(isMalicious) {
       ...
       std::string clear = std::to_string(position.getX()) + "-" +
                           std::to_string(position.getY()) + "-" +
                           std::to_string(position.getZ());
       uint64_t droneNonce = nonceGen.generateNonce();
       uint64_t stationNonce = nonceGen.generateNonce();

       positionResponse->setDroneSignature((generateFakeSignatureEDDSA(clear, droneNonce)).c_str());
       positionResponse->setStationSignature((generateFakeSignatureEDDSA(clear, stationNonce)).c_str());

       positionResponse->setDroneNonce(droneNonce);
       positionResponse->setStationNonce(stationNonce);

    }
    else{
        if (quadNodesPositionTable.hasPosition(address)){
            PositionTable::PositionWithTimestamp p = quadNodesPositionTable.getPositionWithTimestamp(address);
            std::tuple<Coord, std::string, uint64_t, simtime_t> tupla = quadNodesSignatureMap[address];
            Coord posInTuple = std::get<0>(tupla);

            if (posInTuple == p.position){
                positionResponse->setSetted(true);
                positionResponse->setPosition(p.position);
                positionResponse->setTime(p.timestamp);
                uint64_t droneNonce = std::get<2>(tupla);
                std::string droneSignature = std::get<1>(tupla);

                std::string clear = std::to_string(p.position.getX()) + "-" +
                                    std::to_string(p.position.getY()) + "-" +
                                    std::to_string(p.position.getZ());

                uint64_t stationNonce = nonceGen.generateNonce();
                std::string stationSignature;
                if (signAlgorithm == ECDSA){
                    stationSignature = signMessageECDSA(keyPairECDSA, clear, stationNonce);
                }
                else{
                    stationSignature = signMessageEDDSA(clear, privateKeyEDDSA, stationNonce);
                }

                positionResponse->setDroneSignature((droneSignature).c_str());
                positionResponse->setStationSignature((stationSignature).c_str());
    ...
    positionResponse->setChunkLength(B(1 + address.getAddressType()->getAddressByteLength()
            ...
            + sizeof(positionResponse->getDroneSignature())
            + sizeof(positionResponse->getDroneNonce())
            + sizeof(positionResponse->getStationSignature())
            + sizeof(positionResponse->getStationNonce())
            ));
...
void GpsrStation::processPositionResponse(Packet *packet){
    ...
    std::string stationSignature = res->getStationSignature();
    uint64_t stationNonce = res->getStationNonce();
    std::string droneSignature = res->getDroneSignature();
    uint64_t droneNonce = res->getDroneNonce();
    ...
    if(setted){
        ...
        bool droneSignatureVerified = false;
        bool stationSignatureVerified = false;
        std::string senderStation = packet->getSenderModule()->getParentModule()->getFullName();

        if (signAlgorithm == ECDSA){
            droneSignatureVerified = verifySignatureECDSA(keyPairDroneMapECDSA[address], clear, droneNonce, droneSignature);
            stationSignatureVerified = verifySignatureECDSA(keyPairStationMapECDSA[stationIndex], clear, stationNonce, stationSignature);
        }
        else{
            droneSignatureVerified = verifySignatureEDDSA(clear, droneSignature, publicKeyDroneMapEDDSA[address], droneNonce);
            stationSignatureVerified = verifySignatureEDDSA(clear, stationSignature, publicKeyStationMapEDDSA[stationIndex], stationNonce);
        }

        if (droneSignatureVerified && stationSignatureVerified){

            if(count(maliciousStations.begin(), maliciousStations.end(), senderStation) > 0)
                falsePositive++;
            else
                truePositive++;
...
const Ptr<S2SPositionResponse> GpsrStation::createS2SPositionResponse(L3Address applicant, const char* applicantModuleName, L3Address address)
{
    ...
    if(isMalicious) {
        ...
        std::string clear = std::to_string(falsePos.getX()) + "-" +
                            std::to_string(falsePos.getY()) + "-" +
                            std::to_string(falsePos.getZ());
        uint64_t droneNonce = nonceGen.generateNonce();
        uint64_t stationNonce = nonceGen.generateNonce();
...
void GpsrStation::processS2SPositionResponse(Packet *packet){
    ...
    if(setted){
        ...
        if ((droneSignatureVerified && stationSignatureVerified) || isMalicious){
            sendPositionResponse(createPositionResponseFromAnotherStation(address, position, time, droneSignature, droneNonce, stationSignature, stationNonce),
...
void GpsrStation::processPurgeStationTimer()
{
    ...
    for (auto it = quadNodesSignatureMap.begin(); it != quadNodesSignatureMap.end(); ) {
        simtime_t time = std::get<3>(it->second);
        if (time <= timestamp) {
            it = quadNodesSignatureMap.erase(it);
        } else {
            ++it;
        }
    }
...
void GpsrStation::processSendPositionRequest()
{
    ...
    scheduleAfter(uniform(2, 4), sendPositionRequestMessage);
}
...
void GpsrStation::processSelfMessage(cMessage *message)
{
    ...
    else if (message == sendPositionRequestMessage){
        processSendPositionRequest();
    }
    else
...
void GpsrStation::handleStartOperation(LifecycleOperation *operation)
{
    ...
    if(isStation){
        ...
        if (signAlgorithm == ECDSA){
            ...
            generateKeysEDDSA();
            publicKeyStationMapEDDSA[myIndex] = publicKeyEDDSA;
        }

        if(isMalicious){
            std::string station = host->getFullName();
            maliciousStations.push_back(station);
        }
        ...
        if (signAlgorithm == ECDSA){
            generateKeysECDSA();
            keyPairDroneMapECDSA[getSelfAddress()] = keyPairECDSA;
        }
        else{
            generateKeysEDDSA();
            publicKeyDroneMapEDDSA[getSelfAddress()] = publicKeyEDDSA;
        }
...
void GpsrStation::handleStopOperation(LifecycleOperation *operation)
{
    ...
    cancelEvent(sendPositionRequestMessage);
