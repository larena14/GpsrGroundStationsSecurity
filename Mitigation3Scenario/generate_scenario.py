...
def create_ini_file(path_ini, sim_time, minInterval, maxInterval, sendMode, signAlgorithm):
    ini_file = open(path_ini, "w")
    
    # Determina i valori di consumo in base all'algoritmo
    if signAlgorithm.strip().upper() == "ECDSA":
        signPowerConsumption = "2mW"
        verifyPowerConsumption = "1.5mW"
    else:
        signPowerConsumption = "1.5mW"
        verifyPowerConsumption = "1mW"
...
*.node*.energyStorage.initialCapacity =  uniform(47.175Wh, 55.5Wh)
...
*.node*.energyConsumer.signPowerConsumption = {signPowerConsumption}
*.node*.energyConsumer.verifyPowerConsumption = {verifyPowerConsumption}

    """
    ini_file.write(content)
    ini_file.close()

def create_ned_file(grid_size, area_size, malicious_percentage, output_dir, package_name, path_ini, num_msg, m, sim_time, minInterval, maxInterval, sendMode, signAlgorithm):
    ...
    total_stations = len(quadrants_centers)
    num_malicious_stations = math.ceil(malicious_percentage * total_stations)
    malicious_stations = random.sample(range(total_stations), num_malicious_stations)
    
    for i, (x, y) in enumerate(quadrants_centers):
        index = get_station_index(x, y, area_size, m)
        malicious_flag = "true" if i in malicious_stations else "false"
...
def main():
    ...
    parser.add_argument('--nMsg', type=int, required=True, help='Max numero messaggi (FixedMaximum mode)')
    ...
    parser.add_argument('--signAlgorithm', type=str, required=True, help='Sign algorithm (ECDSA o EDDSA)')
    ...
    signAlgorithm = args.signAlgorithm
    
    

    create_ini_file("./omnetpp.ini", sim_time, minInterval, maxInterval, sendMode, signAlgorithm)
    ...
    for area in range(area_min, area_max+1, 1000):
        ...
        for j in range(min_mal, max_mal+1):
            ...
            create_ned_file(grid_size, area, mal_per, f"./{directory_name}", directory_name, "./omnetpp.ini", n_msg, m, sim_time, minInterval, maxInterval, sendMode, signAlgorithm)
