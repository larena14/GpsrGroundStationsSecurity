...
if [ $# -lt 24 ]; then
    echo "Utilizzo: $0 --areaMin <valore> --areaMax <valore> --malMin <valore> --malMax <valore> --nQuad <valore> --nSim <valore> --gridSize <valore> --nMsg <valore> --simTime <valore> --minInterval <valore> --maxInterval <valore> --sendMode <valore>"
    exit 1
fi
...
cd /home/user/omnetpp-6.0.3/samples/Attack2Scenario
...
for ((j=areaMin; j<=areaMax; j+=1000)); do
	for ((i=1; i<=nSim; i++)); do
		...
		./generate_scenario.py --droneGridSize ${gridSize} --areaMin ${areaMin} --areaMax ${j} --malMin ${malMin} \
		        ...
		        --maxInterval ${maxInterval} --sendMode ${sendMode}
	    ...
	    for ((p=malMin; p<=malMax; p+=10)); do 
            while true; do
                ./Attack2Scenario -r 0 -m -u Cmdenv -c Scenario${j}_${p} -n .:../inet4.5/examples:../inet4.5/showcases:../inet4.5/src:../inet4.5/tests/validation:../inet4.5/tests/networks:../inet4.5/tutorials -x inet.common.selfdoc\;inet.linklayer.configurator.gatescheduling.z3\;inet.emulation\;inet.examples.emulation\;inet.showcases.emulation\;inet.transportlayer.tcp_lwip\;inet.applications.voipstream\;inet.examples.voipstream --image-path=../inet4.5/images -l ../inet4.5/src/INET omnetpp.ini

                if [ $? -eq 0 ]; then
                    sleep 2 
                    mv results/json/output.json results/json/Scenario${j}_${p}_${i}_${sendMode}.json
                    break
                else
            ...
            echo "Attack2Scenario: Simulation ${i} finished for Scenario${j}_${p} [mode = ${sendMode}]"
	    done
...
<<comment
Example: 
./launch_simulations.sh --areaMin 1000 --areaMax 1000 --malMin 0 --malMax 40 --nQuad 9 --nSim 40 --gridSize 10 --nMsg 100 --simTime 60 --minInterval 5 --maxInterval 10 --sendMode "Interval" 
./launch_simulations.sh --areaMin 1000 --areaMax 1000 --malMin 0 --malMax 40 --nQuad 9 --nSim 40 --gridSize 10 --nMsg 100 --simTime 60 --minInterval 5 --maxInterval 10 --sendMode "FixedMaximum" 
comment
