...
std::set<L3Address> GpsrStation::maliciousNode;
...
void GpsrStation::processStationNotice(Packet *packet){
    ...
    else{// MODIFICA VA INVIATO E CREATO IL MESSAGGIO CON LA POSIZIONE DEL NODO CERTIFICATA DALLA STAZIONE.
...
void GpsrStation::processBeacon(Packet *packet)
{
    ...
    if(trustness[beaconAddress] <= notTrusted){
             int c=count(maliciousNode.begin(),maliciousNode.end(),beaconAddress);
                   if(c>0){
                           maliciousIndividuati.insert(beaconAddress);
                           //true negative
                           trueNegative += 1.0;
                    }
                    else{
                          falsiPositivi.insert(beaconAddress);
                          falseNegative += 1.0; //false negative
                    }
            delete packet;
     }
     else if(trustness[beaconAddress] >= trusted){
               if (count(maliciousNode.begin(), maliciousNode.end(), beaconAddress) > 0){
                        /* MALICIOUS NODE */
                        falsePositive += 1.0;
               }
               else{
            ...
            if(verified){
                ...
                double measurement = trustness[beaconAddress] + PENALTY;
                trustness[beaconAddress] = (1 - alpha) * trustness[beaconAddress] + alpha * measurement;
                neighborPositionTable.setPosition(beacon->getAddress(), beacon->getPosition());
            }
            else{
                double measurement = trustness[beaconAddress] - PENALTY;
...
void GpsrStation::handleStartOperation(LifecycleOperation *operation)
{
    ...
    if(isStation){
        ...
        if(isMalicious){
                   maliciousNode.insert(getSelfAddress());
              }
