...
cd /home/user/omnetpp-6.0.3/samples/Attack1Scenario
...
for ((j=areaMin; j<=areaMax; j+=1000)); do
	for ((i=1; i<=nSim; i++)); do
	    ...
	    for ((p=malMin; p<=malMax; p+=10)); do 
            while true; do
                ./Attack1Scenario -r 0 -m -u Cmdenv -c Scenario${j}_${p} -n .:../inet4.5/examples:../inet4.5/showcases:../inet4.5/src:../inet4.5/tests/validation:../inet4.5/tests/networks:../inet4.5/tutorials -x inet.common.selfdoc\;inet.linklayer.configurator.gatescheduling.z3\;inet.emulation\;inet.examples.emulation\;inet.showcases.emulation\;inet.transportlayer.tcp_lwip\;inet.applications.voipstream\;inet.examples.voipstream --image-path=../inet4.5/images -l ../inet4.5/src/INET omnetpp.ini
                if [ $? -eq 0 ]; then
                    sleep 2 
